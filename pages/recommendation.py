...
def recommendation(temp, gender, closet):
    ...
    if weather_category[:4] == 'warm':
        if weather_category == 'warm1':
            ...
            clothes_kind = {'top':['긴팔', '후드티', '맨투맨'],
                            'bottom':['면바지', '슬렉스', '청바지', '치마'],
                            'shoes':['운동화', '워커']}
        else:
            clothes_kind = {'top':['긴팔', '니트', '후드티', '맨투맨'],
                            'bottom':['면바지', '청바지', '슬렉스', '치마'],
                            'one':['원피스'],
                            'shoes':['운동화', '워커']}
    else:
        if weather_category == 'cool1':
            clothes_kind = {'top':['셔츠', '긴팔'],
                            'bottom':['면바지', '청바지', '슬렉스', '치마'],
                            'shoes':['운동화', '워커'],
                            'outer':['자켓', '가디건']}
        elif weather_category == 'cool2':
            clothes_kind = {'top':['셔츠', '긴팔', '니트'],
                            'bottom':['청바지', '치마'],
                            'shoes':['운동화', '워커'], 
                            'outer':['자켓', '집업']}
    ...
    # 남성일 경우 여자 옷 제거
    if gender == '남성':
        if 'one' in clothes_kind.keys():
            clothes_kind.pop('one', None)
        if '치마' in clothes_kind['bottom']:
            clothes_kind['bottom'] = [k for k in clothes_kind['bottom'] if k != '치마']
    
    # 옷 선택지에 '원피스'가 있고 옷장에 '원피스'가 있을 경우, 원피스와 topbottom 중에 random sample
    if 'one' in clothes_kind.keys():
        if 'one' in closet.category.values:
            choice = random.sample(['one', 'topbottom'], 1)[0]
        else:
            choice = ['topbottom']
    else:
        choice = ['topbottom']
